            ...
            ?>
                <div class="col-12 mt-3 mb-3">
                    <div class="row">
                        ...
                        <?php $Profile_image_rs = Database::search("SELECT * FROM `profile_image` WHERE user_email='" . $selected_data['email'] . "'");
                                    $Profile_image_num = $Profile_image_rs->num_rows;
                                    $Profile_image_data = $Profile_image_rs->fetch_assoc(); 
                                    if($Profile_image_num != 0){
                                    ?>
                            <img src="<?php echo $Profile_image_data['path'];?>" style="height: 40px;margin-left: 80px;" />
                            <?php
                                    }else{
                                        ?>
                                        ...
                                        <div class="col-8 rounded bg-success">
                                            <div class="row">
                                                <div class="col-12 pt-2">
                                                    ...
                                                    <?php
                                                    $c = Database::search("SELECT * FROM `chat2` where `from` = '".$selected_data["email"]."'ORDER BY `date_time` DESC"); 
                                                    $cn = $c->num_rows;
                                                  
                                                        $c_data = $c->fetch_assoc();
                                                        if ($c_data["status"] == 0) {
                                                    ?>
                                                    <span class="text-white fw-bold fs-4"><?php echo $c_data["content"]; ?></span>
                                                </div>
                                                <div class="col-12 text-end pb-2">
                                                <span class="text-white fs-6"><?php echo $c_data["date_time"]; ?></span>
                                                </div>
                                            </div>
                                        </div>
                                    </div>
                                    ...
                                    <div class="row">
                                        <div class="offset-4 col-8 rounded bg-primary">
                                            <div class="row">
                                                <div class="col-12 pt-2">
                                                <?php 
                                                    $a = Database::search("SELECT * FROM `admin`");
                                                    $a_d=$a->fetch_assoc();
                                                    $r = Database::search("SELECT * FROM `chat1` where `from` = '".$a_d["email"]."'ORDER BY `date_time` DESC"); 
                                                    $rn = $r->num_rows;
                                                    $r_data = $r->fetch_assoc();
                                                        if ($r_data["status"] == 0) {
                                                    ?>
                                                    <span class="text-white fw-bold fs-4"><?php echo $r_data["content"]; ?></span>
                                                </div>
                                                <div class="col-12 text-end pb-2">
                                                <span class="text-white fs-6"><?php echo $r_data["date_time"];?></span>
                                                </div>
                                            </div>
                                        </div>
                                    </div>
